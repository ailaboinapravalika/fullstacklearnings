...
class AppStore extends Component {
  ...
  render() {
    ...
    const newAppsList = appsList.filter(app => app.category === activeTabId)
    const filteredAppsList = newAppsList.filter(app =>
      app.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="bg-appstore">
        ...
        <ul className="apps-container">
          {filteredAppsList.map(app => (
            <AppItem appData={app} key={app.appId} />
          ))}
        </ul>
      </div>
    )
